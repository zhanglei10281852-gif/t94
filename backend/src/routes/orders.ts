    ...
    try {
      ...
      if (status === "confirmed" && previousStatus !== "confirmed") {
        ...
        if (!order.isPaid) {
          ...
          if (!account) {
            return res
              .status(400)
              .json({ message: "该老人未开通就餐账户，请先开通或充值" });
          }

          if (account.status !== "normal") {
            return res.status(400).json({ message: "账户状态异常，无法扣费" });
          }

          const payAmount = order.selfPayAmount;
          const totalBalance = account.balance + account.giftBalance;

          if (totalBalance < payAmount) {
            return res.status(400).json({
              message: `账户余额不足，当前余额${totalBalance}元，应付${payAmount}元，请先充值`,
              currentBalance: totalBalance,
              needAmount: payAmount,
            });
          }
          ...
          if (account.giftBalance >= payAmount) {
            giftDeduction = payAmount;
            account.giftBalance -= payAmount;
          } else {
            giftDeduction = account.giftBalance;
            balanceDeduction = payAmount - account.giftBalance;
            account.giftBalance = 0;
            account.balance -= balanceDeduction;
          }

          account.totalConsume += payAmount;
          account.totalSubsidyDeduction += order.subsidyAmount;
          account.lastConsumeAt = new Date();

          const consumeTransaction = new AccountTransaction({
            ...
            remark: `餐费 ${order.mealPrice}元，补贴 ${order.subsidyAmount}元，实扣 ${payAmount}元（赠送${giftDeduction}元+充值${balanceDeduction}元）`,
            operatorId: req.user?._id,
          });

          const subsidyTransaction = new AccountTransaction({
            accountId: account._id,
            elderlyId: order.elderlyId,
            type: "subsidy_deduction",
            amount: order.subsidyAmount,
            balanceBefore,
            balanceAfter: account.balance,
            giftBalanceBefore,
            giftBalanceAfter: account.giftBalance,
            relatedOrderId: order._id,
            remark: `补贴抵扣 ${order.subsidyAmount}元`,
            operatorId: req.user?._id,
          });

          await account.save();
          await consumeTransaction.save();
          await subsidyTransaction.save();

          order.isPaid = true;
          order.paidAmount = balanceDeduction;
          order.paidGiftAmount = giftDeduction;
      ...
      if (status === "cancelled" && previousStatus !== "cancelled") {
        if (order.isPaid && !order.isRefunded) {
          ...
          if (account) {
            const refundAmount = order.paidAmount + order.paidGiftAmount;
            const balanceBefore = account.balance;
            const giftBalanceBefore = account.giftBalance;

            account.balance += order.paidAmount;
            account.giftBalance += order.paidGiftAmount;
            account.totalRefund += refundAmount;
            account.totalConsume -= refundAmount;
            account.totalSubsidyDeduction -= order.subsidyAmount;

            const refundTransaction = new AccountTransaction({
              accountId: account._id,
              elderlyId: order.elderlyId,
              type: "refund",
              amount: refundAmount,
              ...
              giftBalanceAfter: account.giftBalance,
              relatedOrderId: order._id,
              remark: `订单取消退款：${refundAmount}元（充值${order.paidAmount}元+赠送${order.paidGiftAmount}元）`,
              operatorId: req.user?._id,
            });
